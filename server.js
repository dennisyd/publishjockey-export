...
// POST /api/uploads (for WYSIWYG image upload)
app.post('/api/uploads', (req, res) => {
  // Use a specific multer instance configured for this endpoint
  const upload = multer({
    storage: multer.diskStorage({
      destination: (req, file, cb) => {
        // Create the directory structure for admin/test
  ...
  upload(req, res, function(err) {
    ...
    // Return the path in the format expected by the frontend image tag
    const relPath = `uploads/admin/test/${filename}`;
    console.log('Returning image path:', relPath);
