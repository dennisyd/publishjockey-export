...
% --- HINDI LANGUAGE SETUP WITH PROPER FONT FALLBACK ---
$if(polyglossia)$
\ifxetex
  ...
  \setsansfont{$if(sansfont)$$sansfont$$else$Liberation Serif$endif$}[
    $if(sansfontoptions)$$sansfontoptions$,$endif$
    Script=Latin,
    Ligatures=TeX,
    Scale=MatchLowercase
  ]
  
  \setseriffont{$if(seriffont)$$seriffont$$else$Liberation Serif$endif$}[
    $if(seriffontoptions)$$seriffontoptions$,$endif$
    Script=Latin,
    Ligatures=TeX,
    Scale=MatchLowercase
  ]
...
$endif$
$if(seriffont)$
    \setseriffont[$for(seriffontoptions)$$seriffontoptions$$sep$,$endfor$]{$seriffont$}
$endif$
$endif$
