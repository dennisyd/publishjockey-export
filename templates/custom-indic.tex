...
\ifxetex
  ...
  % Set English as default since we're using ucharclasses for script switching
  % This avoids polyglossia language-specific config issues
  \setdefaultlanguage{english}
  
  % Configure Indic languages as "other" languages with proper settings
  $if(lang)$
  $if(script-language)$
  % Set up the Indic language with proper hyphenation settings
  \setotherlanguage{$script-language$}
  % Disable hyphenation for Indic scripts (they don't hyphenate like Latin)
  \PolyglossiaSetup{$script-language$}{hyphenmins={100,100}}
  $endif$
  $endif$
