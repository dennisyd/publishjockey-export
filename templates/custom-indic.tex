...
\ifxetex
  ...
  \newfontfamily\englishfont{$if(romanfont)$$romanfont$$else$Liberation Serif$endif$}[
    Renderer=HarfBuzz,
    Ligatures={Required,Common},
  ...
  % Monospace font
  \setmonofont{$if(monofont)$$monofont$$else$Liberation Mono$endif$}[
    Scale=MatchLowercase
  ]
  
  % Conservative microtype settings (XeLaTeX compatible)
...
% PUBLISHER-GRADE TYPOGRAPHY for Indic scripts
\linespread{1.10}  % Optimized for Indic script readability
...
% Disable hyphenation for Indic scripts (linguistically inappropriate)
\hyphenpenalty=10000
...
% Better page breaking and natural line endings
\raggedbottom
\parfillskip=0pt plus 0.5\textwidth  % Allow natural right margin instead of forcing full justification
